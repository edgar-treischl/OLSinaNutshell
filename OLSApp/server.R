...
shinyServer(function(input, output) {
    ...
    observeEvent(input$button1, {
        ...
        output$text1 <- renderText({
            paste("Look at the intercept and the beta-coefficient:", model()[1])
        })
        output$text2 <- renderText({
            paste("The intercept tells you the predicted value of y in case x is 0. What does this mean?")
        })
        output$text3 <- renderText({
            paste("The beta-coefficient tells you how much y is increased (decreased) if x goes up by 1 unit. Use both to calculate the predicted value.")
        })
    })
...
We have to look at the summary statistics to get an idea about the effect size.")
    ...
    observeEvent(input$button3, {
        ...
        output$text5 <- renderText({
            paste("We assume that there is a linear association between X and Y. 
Certainly, this must not be the case and the OLS assumption(s) might be violated.")
        })
    })
